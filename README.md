...
![OpenBake 메인 화면 — 한정 드롭 상품 목록](assets/readme/openbake-main.png)

**로컬 베이커리의 일반 상품과 한정판 디저트를 구매하는 드롭 커머스 플랫폼**

OpenBake는 정해진 시간과 수량으로 판매하는 **드롭(Drop)**, 일반 상품 주문, 매장 픽업을 지원합니다.
드롭 입장과 Redis 재고 선점으로 구매 흐름을 제어하고, 예치금 결제부터 구매확정·판매자 정산까지 연결합니다.

이 저장소는 베이커리6단지 팀의 백엔드 프로젝트입니다. 핵심 커머스 서버와 회원·결제·추천 서비스,
API Gateway를 Gradle 멀티 모듈로 관리합니다.
