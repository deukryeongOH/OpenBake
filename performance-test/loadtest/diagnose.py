...
def diagnose(summary: dict[str, dict[str, float]],
             ...
             scenario: str) -> tuple[list[Finding], list[str]]:
    ...
    # --- 6. HPA / replica ---
    ready_values = numbers(rows, "replicas_ready")
    desired_values = numbers(rows, "replicas_desired")
    hpa_cur = peak(rows, "hpa_current_pct")
    hpa_target = peak(rows, "hpa_target_pct")
    replica_min = min(ready_values) if ready_values else None
    replica_max = max(ready_values) if ready_values else None
    scaled = bool(ready_values) and replica_max != replica_min

    findings.append(Finding(
        hit=(hpa_cur is not None and hpa_target is not None
             and hpa_cur > hpa_target and not scaled),
        title="HPA 미반응",
        evidence=f"replica {fmt(replica_min, digits=0)} -> {fmt(replica_max, digits=0)}, "
                 f"HPA {fmt(hpa_cur, '%', 0)} / 목표 {fmt(hpa_target, '%', 0)}",
        advice="CPU 사용률이 목표를 넘었는데 replica 가 늘지 않았다.\n"
               "이 시나리오는 per-vu-iterations 버스트라 수 초에 끝난다. HPA 는\n"
               "metrics-server 수집 주기 + scaleUp 30초당 1개 + startupProbe 최대 240초가\n"
               "필요해 애초에 따라올 수 없다. 오토스케일 효과를 보려면 분 단위로\n"
               "지속되는 시나리오가 따로 필요하다.",
    ))

    if replica_max is not None and replica_max >= 2:
        notes.append(f"replica 가 {replica_max:.0f}개까지 늘었다. "
                     f"HPA maxReplicas 가 2 이므로 상한에 도달했다.")

    # --- 7. 응답시간 분해 (서버 내부 vs 네트워크) ---
    ...
    if waiting is not None and p95:
        ...
        findings.append(Finding(
            ...
            advice="지연의 30% 이상이 서버 처리 밖(연결 수립·대기)에서 발생했다.\n"
                   "k6 를 측정 대상과 같은 호스트에서 돌렸다면 부하 생성기가 CPU 를\n"
                   "나눠 먹은 것일 수 있다. 별도 호스트에서 재측정할 것.",
        ))
...
def main() -> int:
    ...
    verdict = "FAIL" if (p95_over or p99_over) else "PASS"

    throughput = (summary.get("iterations", {}) or {}).get("rate")
    req_rate = (summary.get("http_reqs", {}) or {}).get("rate")
    total_reqs = (summary.get("http_reqs", {}) or {}).get("count")

    findings, notes = diagnose(summary, rows, args.scenario)
    ...
    L.append(f"   P99  {fmt(p99, 'ms', 0)}   {'초과' if p99_over else '통과'}")
    ...
    if ready:
        L.append(f"   replica       {min(ready):.0f} -> {max(ready):.0f}"
                 f"   (HPA {fmt(peak(rows, 'hpa_current_pct'), '%', 0)} / "
                 f"목표 {fmt(peak(rows, 'hpa_target_pct'), '%', 0)}, max 2)")
    L.append("")
    L.append(" [정합성]")
    L.extend(outcome)
    ...
    L.append("")
    L.append(" [알려진 측정 왜곡]")
    L.append("   - backend-config.yaml 에 SPRING_JPA_SHOW_SQL 오버라이드가 없어")
    L.append("     application.yml 의 show-sql=true, hibernate.orm.jdbc.bind=trace 가 살아 있다.")
    L.append("     모든 요청이 쿼리와 바인딩 파라미터를 stdout 에 쓴다.")
