...
public class CartController {
    ...
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "C001 잘못된 요청입니다. (productId 누락, quantity < 1) / CA004 선택할 수 없는 픽업 날짜입니다."),
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "409", description = "CA005 이미 지난 픽업 날짜입니다. / CA009 남은 재고보다 많은 수량을 담을 수 없습니다.")
    })
    @PostMapping("/items")
    ...
    @Operation(
            summary = "장바구니 조회",
            description = "장바구니 화면을 그릴 때 호출합니다. 장바구니를 만든 적이 없거나 비어 있어도 200으로 빈 목록을 내려줍니다. 상품명·가격·이미지·재고·픽업 가능일·판매자 상호명은 스냅샷이 아니라 조회 시점의 최신값이라, 담아둔 뒤 판매자가 가격이나 상호를 바꾸면 값이 달라집니다. 가격이 담을 때와 달라진 항목은 priceChanged=true와 담을 때 단가(addedPrice)를 함께 내려주므로 'addedPrice → price' 형태로 변동 폭을 안내할 수 있습니다(오른 경우와 내린 경우 모두). 결제 금액은 언제나 최신 price 기준입니다. 상품이 삭제됐거나, 재고가 담아둔 수량보다 적거나, 고른 픽업 날짜가 더 이상 선택 가능일이 아닌 항목은 orderable=false와 status로 사유를 함께 내려주니 프론트가 해당 항목과 주문 버튼을 비활성 처리하면 됩니다. 판매자가 픽업 가능일을 추가·수정하면 pickUpAvailableDates에 바로 반영되므로 그중에서 다시 고르면 됩니다. totalAmount는 주문 가능한 항목만 합산합니다."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "ME002 유효하지 않은 인증 토큰입니다.")
    })
