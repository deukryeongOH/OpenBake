...
public class DropLockFacade {
    ...
    public void reserveStock(Long dropId, Long memberId, DropReserveRequest request) {
        // 1인당 제한 수량과 선택 수량 검증
        dropLockService.checkLimitQuantityPerPerson(dropId, request.quantity());
        // 대기열에 통과된 직후인지 상태 검증
        ...
        try{
            ...
            // 락을 획득했으니 재고 차감
            dropLockService.decreaseQuantity(dropId, memberId, request.quantity());
