...
public record CachedDrop(LocalDate cachedDate, Long dropId, Long productId, LocalDateTime dropStart, LocalDateTime dropEnd, AtomicBoolean checkStarted, AtomicBoolean checkEnded) {
    static CachedDrop of(Drop drop) {
        return new CachedDrop(LocalDate.now(), drop.getId(), drop.getProductId(), drop.getDropStart(), drop.getDropEnd(), new AtomicBoolean(false), new AtomicBoolean(false));
    }
