...
/**
 * 자동 구매확정 배치. 결제 완료 후 지정 일수가 지나도록 확정되지 않은 주문 항목을 자동 확정한다.
 * 판매자가 확정을 누르지 않아도 정산이 누락되지 않게 하는 안전망이다.
 *
 * 대상 조회와 확정을 분리해, 항목 하나가 실패해도 나머지는 계속 처리한다(항목별 트랜잭션).
 */
@Component
...
public class AutoConfirmScheduler {
    ...
    public void run() {
        List<Long> targetIds = confirmService.findAutoConfirmTargetItemIds();
        ...
        for (Long orderItemId : targetIds) {
            try {
                confirmService.autoConfirmItem(orderItemId);
                confirmed++;
            } catch (Exception e) {
                //한 건 실패가 배치 전체를 막지 않도록 로그만 남기고 계속 진행한다.
                log.error("자동 구매확정 실패 orderItemId={}, reason={}", orderItemId, e.getMessage());
            }
        }
